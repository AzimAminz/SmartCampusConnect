package smartcampus.booking.config;
...
import smartcampus.booking.service.CampusBookingSoapService;

/**
 * Publishes the JAX-WS SOAP Endpoint on port 8085.
        ...
        Thread soapThread = new Thread(() -> {
            try {
                ...
                context.getFilters().add(new com.sun.net.httpserver.Filter() {
                    ...
                    @Override
                    public void doFilter(com.sun.net.httpserver.HttpExchange exchange, Chain chain)
                ...
                System.out.println("✅ [SOAP-SERVER] WSDL published at: http://localhost:" + soapPort + "/ws/booking?wsdl");

                synchronized (this) { this.wait(); }
            } catch (Exception e) {
                System.err.println("❌ [SOAP-SERVER] Failed: " + e.getMessage());
                e.printStackTrace();
            }
        });
